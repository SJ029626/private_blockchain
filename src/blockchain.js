...
class Blockchain {
    ...
    requestMessageOwnershipVerification(address) {
        return new Promise((resolve) => {
            let owner_message = `${address}:${new Date().getTime().toString().slice(0,-3)}:starRegistry`;
            resolve(owner_message);
        });
    }
    ...
    submitStar(address, message, signature, star) {
        let self = this;
        return new Promise(async (resolve, reject) => {
           let theTime = parseInt(message.split(':')[1]);
           let nowTime = parseInt(new Date().getTime().toString().slice(0, -3));
           if((theTime+(5*60*1000)) >= nowTime){
                ...
                reject('You should submit the star in 5 minutes');
            }
        });
    }
    ...
    validateChain() {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            for (const block of self.chain) {
                ...
                const result = block.previousBlockHash !== prevBlockHash
                if (!isValid || result) {
                    errorLog.push({
                        block,
                        error: "Validation of block is not be able to done."
                    })
                }
